    ...
    3: [
      function (require, module, exports) {
        ...
        function createLogo(options = {}) {
          ...
          const renderFox = createModelRenderer(
            container,
            cameraDistance,
            modelObj
          );
    ...
    13: [
      function (require, module, exports) {
        ...
        function createModelRenderer(container, cameraDistance, modelObj) {
          ...
          const updateFaces = createFaceUpdater(
            container,
            polygons,
            transformed
          );
         ...
         */
        function createStandardModelPolygon(
          chunk,
          { gradients = {}, index, masks }
        ) {
        ...
        function calculateSizingOptions(options = {}) {
          ...
          if (!options.pxNotRatio) {
            ...
            if ("minWidth" in options && width < options.minWidth) {
              ...
              height = Math.floor(
                (options.minWidth * options.height) / options.width
              );
            }
          }
          return { width, height };
        ...
        function setGradientDefinitions(container, gradients) {
          ...
          const radialCoordinateAttributes = [
            "cx",
            "cy",
            "fr",
            "fx",
          ...
          for (const [gradientId, gradientDefinition] of Object.entries(
            gradients
          )) {
            let gradient;
            if (gradientDefinition.type === "linear") {
              ...
              const unsupportedLinearAttribute = Object.keys(
                gradientDefinition
              ).find((attribute) => !allLinearAttributes.includes(attribute));
              if (unsupportedLinearAttribute) {
                ...
                linearCoordinateAttributes.some(
                  (attributeName) =>
                    gradientDefinition[attributeName] !== undefined
                )
              ) {
              ...
              const presentCoordinateAttributes =
                radialCoordinateAttributes.filter(
                  (attributeName) =>
                    gradientDefinition[attributeName] !== undefined
                );
              const unsupportedRadialAttribute = Object.keys(
                gradientDefinition
              ).find((attribute) => !allRadialAttributes.includes(attribute));
            ...
            if (gradientDefinition.spreadMethod !== undefined) {
              if (
                !["pad", "reflect", "repeat"].includes(
                  gradientDefinition.spreadMethod
                )
              ) {
                throw new Error(
